...
export const authAPI = {
  async register(name: string, email: string, password: string): Promise<ApiResult<{id: string}>> {
    try {
      ...
      return { 
        data: null, 
        error: isAPIError(error) ? error : { message: 'Unknown error during registration' },
        loading: false
      };
    }
  },

  async checkPassword(password: string): Promise<ApiResult<PasswordCheckResponse>> {
    try {
      const response = await axiosClient.post('/api/auth/check-password', { password });
      return { data: response.data, error: null, loading: false };
    } catch (error) {
      return { 
        data: null, 
        error: isAPIError(error) ? error : { message: 'Error checking password security' },
        loading: false
      };
    }
  },
  ...
  async forgotPassword(email: string): Promise<ApiResult<{message: string}>> {
    try {
      ...
      return { 
        data: null, 
        error: isAPIError(error) ? error : { message: 'Error processing password reset' },
        loading: false
      };
    }
  },

  async validateResetToken(token: string): Promise<ApiResult<{userId: string}>> {
    try {
      ...
      return { 
        data: null, 
        error: isAPIError(error) ? error : { message: 'Invalid or expired token' },
        loading: false
      };
    }
  },
  ...
  async resetPassword(token: string, password: string): Promise<ApiResult<{message: string}>> {
    try {
      ...
      return { 
        data: null, 
        error: isAPIError(error) ? error : { message: 'Error resetting password' },
        loading: false
      };
    }
  },
...
export const profileAPI = {
  ...
  async updateProfile(data: { name: string }): Promise<ApiResult<ProfileData>> {
    try {
      ...
      return { 
        data: null, 
        error: isAPIError(error) ? error : { message: 'Error updating profile' },
        loading: false
      };
    }
  },
  ...
  async deleteAccount(): Promise<ApiResult<{message: string}>> {
    try {
      ...
      return { 
        data: null, 
        error: isAPIError(error) ? error : { message: 'Error deleting account' },
        loading: false
      };
    }
  },
  ...
  async getStats(): Promise<ApiResult<UserStats>> {
    try {
      ...
      return { 
        data: null, 
        error: isAPIError(error) ? error : { message: 'Error loading statistics' },
        loading: false
      };
    }
  },
...
export const todoAPI = {
  ...
  async updateTodo(id: string, todoData: Partial<Todo>): Promise<ApiResult<Todo>> {
    try {
